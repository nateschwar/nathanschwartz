...
import { galleries, getGalleryTags } from "@/lib/gallery-data";

export const Route = createFileRoute("/gallery")({
  head: () => ({
    meta: [
      { title: "Gallery - Galleries" },
...
function Gallery() {
  const isGalleryPage = useRouterState({
    select: (state) => state.location.pathname !== "/gallery",
  });
        ...
        <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredGalleries.map((gallery) => (
            <article key={gallery.id} className="group bg-white shadow-sm">
              <Link to="/gallery/$galleryId" params={{ galleryId: gallery.id }} className="block">
