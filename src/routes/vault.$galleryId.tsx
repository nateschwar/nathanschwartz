...
import { getGallery, slugify } from "@/lib/vault-data";

export const Route = createFileRoute("/vault/$galleryId")({
  head: ({ params }) => {
    const gallery = getGallery(params.galleryId);

    return {
      meta: [
        { title: gallery ? `${gallery.title} - Vault` : "Gallery Not Found" },
...
function GalleryPage() {
  ...
  const visibleImages = gallery && isUnlocked ? gallery.images : [];
